...
- **R4 — CORRECTED ON RE-AUDIT (PW-0402). It said "no authentication or authorization exists on any API route yet"; half of that is out of date and the half that is still true is the more serious one.**
  ...
  **AUTHENTICATION IS NOT, and that is the real residual.** `resolveRequestAccount` establishes an account from a **plaintext development header** in a non-deployment runtime, and refuses with `authentication_not_configured` in a deployment. So the routes are not anonymous: they are **unauthenticated in development and closed in production**. That is a narrower gap than the original wording, and a sharper one — it means the identity every authorization decision is built on is, today, whatever the caller typed in a header, on any machine where the runtime classifies as non-deployment.
  ...
  **STILL OPEN:** wiring a real authenticated session provider behind `resolveRequestAccount`. ADR-007 selected Better Auth and is still *Proposed*. Until that lands, a desktop build must not classify as a non-deployment runtime, or the development header becomes the identity layer.
