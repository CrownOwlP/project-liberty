 ...
 *   - the decision is read from a STRUCTURED field, never parsed from prose
 ...
 *     explicitly excluded from the verdict (commits can sweep in unrelated work)
 *   - test evidence is never invented: the worker reports what CI recorded
...
import { listMessages, acknowledge, readRejection, rejectMessage } from "../agent-bus.mjs";
...
const DECISION_SCHEMA = {
  ...
  properties: {
    ...
    reviewedScopeConfirmed: {
      type: "boolean",
      description: "True only if the verdict is based solely on files inside the task's allowedPaths."
    },
    blockingFindings: {
      type: "array",
      items: {
...
async function callOpenAI(payload, attempt = 1) {
  ...
  try {
    ...
    if (!response.ok) {
      throw Object.assign(new Error(`OpenAI error ${response.status}: ${(await response.text()).slice(0, 800)}`), { fatal: true });
    }
...
function extractStructured(result) {
  // Responses API: prefer the parsed structured payload; fall back to the
  // aggregated text, which is still JSON because of the schema constraint.
...
function buildReviewContext(task, commitSha) {
  const allowed = task.allowedPaths ?? [];
  const changed = git("diff", "--name-only", `${commitSha}~1`, commitSha)
  ...
  let patch = "";
  if (inScope.length) {
    patch = git("diff", `${commitSha}~1`, commitSha, "--", ...inScope);
    if (patch.length > MAX_PATCH_BYTES) {
      patch = patch.slice(0, MAX_PATCH_BYTES) + "\n[... diff truncated for length ...]\n";
    }
  }
  return { inScope, outOfScope, patch };
}
...
async function reviewMessage(message) {
  ...
  const { inScope, outOfScope, patch } = buildReviewContext(task, message.commitSha);
  ...
  const instructions = [
    ...
    "  into the same commit by another task and MUST NOT influence your verdict.",
    "- Never fabricate test evidence. If you need a test result you do not have, that is a",
    "  blocking finding, not an assumption.",
  ...
  const input = [
    ...
    "",
    `## Reviewing commit ${message.commitSha}`,
    `Request summary: ${message.summary}`,
    `Evidence supplied by the implementer: ${JSON.stringify(message.evidence ?? [])}`,
    "",
    `## In-scope changed files (${inScope.length})`,
    inScope.join("\n") || "(none)",
    "",
    `## OUT OF SCOPE - context only, must not affect the verdict (${outOfScope.length})`,
    outOfScope.join("\n") || "(none)",
    "",
    "## Diff (in-scope only)",
    "```diff",
    patch || "(no in-scope changes)",
    "```",
    "",
  ...
  return { task, decision, inScope, outOfScope };
}

function publishDecision(message, task, decision, scope) {
  const evidence = [
    `reviewed=${message.commitSha}`,
    `inScopeFiles=${scope.inScope.length}`,
    `outOfScopeIgnored=${scope.outOfScope.length}`,
    `model=${MODEL}`,
...
for (const message of pending) {
  ...
  try {
    const { task, decision, inScope, outOfScope } = await reviewMessage(message);
    const publishedId = publishDecision(message, task, decision, { inScope, outOfScope });
    // Acknowledge only AFTER the decision is durably published, so a crash
    // in between leaves the request pending rather than silently consumed.
    ...
    // A defect in the request itself is quarantined; anything else is left
    // pending for the next scheduled run.
    if (/unknown task|must not review its own work/.test(error.message)) {
